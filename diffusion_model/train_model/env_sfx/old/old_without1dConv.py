...
#         self.cond_proj = nn.Sequential(nn.Linear(c_dim, out_ch*2),nn.LayerNorm(out_ch*2)) if c_dim else None
#         self.nin_shortcut = nn.Conv2d(in_ch, out_ch, 1) if in_ch != out_ch else None

#         # 可学习的放大因子，方便控制条件强度
#         if c_dim:
#             self.cond_scale = nn.Parameter(torch.ones(1) * 1.0)
#         else:
...
#             scale_t = torch.tanh(scale_t).unsqueeze(-1).unsqueeze(-1)  # tanh -> [-1,1]
...
#                 scale_c = scale_c * self.cond_scale
#                 shift_c = shift_c * self.cond_scale
...
class ImprovedUNetCond(nn.Module):
    def __init__(self, in_channels=1, base_ch=256, t_dim=512, c_dim=512):
        ...
        # self.text_mlp = nn.Sequential(nn.Linear(c_dim, c_dim), nn.SiLU(), nn.Linear(c_dim, c_dim))
        self.time_mlp = nn.Sequential(nn.Linear(t_dim, t_dim), nn.LayerNorm(t_dim), nn.SiLU(), nn.Linear(t_dim, t_dim))
        # encoder
        ...
        self.enc5 = ResBlock(base_ch*8, base_ch*16, t_dim)
        self.mid = ResBlock(base_ch*16, base_ch*16, t_dim)
        # decoder
        self.dec5 = ResBlock(base_ch*32, base_ch*8, t_dim)
        ...
        # self.cemb_layers = 11  # enc1~enc5 + mid
        # self.text_cond_mlps = nn.ModuleList([
        #     nn.Sequential(
        #         nn.Linear(c_dim, c_dim),
        #         nn.SiLU(),
        #         nn.Linear(c_dim, c_dim)
        #     ) for _ in range(self.cemb_layers)
        # ])
        ...
        self.c_weight = nn.Parameter(torch.tensor(1.0))

    def forward(self, x, t, c_emb=None):
        c_emb = self.text_proj(c_emb)
        c_emb = F.normalize(c_emb, dim=-1)
        # c_emb = self.text_mlp(c_emb)
        # c_embs = [mlp(c_emb) for mlp in self.text_cond_mlps]
        t_emb = sinusoidal_embedding(t, self.t_dim)
        t_emb = self.time_mlp(t_emb)
        t_emb = F.normalize(t_emb, dim=-1)
        cond = self.t_weight * t_emb + self.c_weight * c_emb
        e1 = self.enc1(x, cond)
        e2 = self.enc2(self.pool(e1), cond)
        e3 = self.enc3(self.pool(e2), cond)
        e4 = self.enc4(self.pool(e3), cond)
        e5 = self.enc5(self.pool(e4), cond)
        m = self.mid(self.pool(e5), cond)

        d5 = self.upsample(m)
        d5 = self.dec5(align_and_cat(d5, e5), cond)

        d4 = self.upsample(d5)
        d4 = self.dec4(align_and_cat(d4, e4), cond)

        d3 = self.upsample(d4)
        d3 = self.dec3(align_and_cat(d3, e3), cond)

        d2 = self.upsample(d3)
        d2 = self.dec2(align_and_cat(d2, e2), cond)

        d1 = self.upsample(d2)
        d1 = self.dec1(align_and_cat(d1, e1), cond)
    ...
    for epoch in range(args.epochs):
        ...
        for i, (mel, text) in enumerate(pbar):
            ...
            if args.mixed_precision:
                ...
                global_step += 1
                pbar.set_postfix(loss=loss.item(),diff=diff.item(),std=eps_theta.std().item(),mean=eps_theta.mean().item())
            ...
            #         name = f'block_{i}'
            #         if hasattr(block, 'cond_proj'):
            #             p = block.cond_proj.weight
            #             print(f"{name} cond_proj weight.requires_grad={p.requires_grad}, grad is None? {p.grad is None}")
            #             if p.grad is not None:
            #                 print(f"  grad mean_abs={p.grad.abs().mean().item():.6e}, max={p.grad.abs().max().item():.6e}")
